...
describe('Query params', () => {
    ...
    before((done) => {
        ...
        async.parallel([
            (cb) => {
                utils_1.default.loadApi(google, 'drive', 'v2', {}, cb);
            },
            (cb) => {
                utils_1.default.loadApi(google, 'gmail', 'v1', {}, cb);
            }
        ], (err, apis) => {
            if (err) {
                return done(err);
    ...
    it('should not append ? with no query parameters', () => {
        let uri = localDrive.files.get({ fileId: 'ID' }, utils_1.default.noop).uri;
        assert.equal(-1, uri.href.indexOf('?'));
        uri = remoteDrive.files.get({ fileId: 'ID' }, utils_1.default.noop).uri;
        assert.equal(-1, uri.href.indexOf('?'));
    });
    it('should be null if no object passed', () => {
        let req = localDrive.files.list(utils_1.default.noop);
        assert.equal(req.uri.query, null);
        req = remoteDrive.files.list(utils_1.default.noop);
        assert.equal(req.uri.query, null);
    });
    it('should be null if params passed are in path', () => {
        let req = localDrive.files.get({ fileId: '123' }, utils_1.default.noop);
        assert.equal(req.uri.query, null);
        req = remoteDrive.files.get({ fileId: '123' }, utils_1.default.noop);
        assert.equal(req.uri.query, null);
    });
    it('should be set if params passed are optional query params', () => {
        let req = localDrive.files.get({ fileId: '123', updateViewedDate: true }, utils_1.default.noop);
        assert.equal(req.uri.query, 'updateViewedDate=true');
        req = remoteDrive.files.get({ fileId: '123', updateViewedDate: true }, utils_1.default.noop);
        assert.equal(req.uri.query, 'updateViewedDate=true');
    });
    it('should be set if params passed are unknown params', () => {
        let req = localDrive.files.get({ fileId: '123', madeThisUp: 'hello' }, utils_1.default.noop);
        assert.equal(req.uri.query, 'madeThisUp=hello');
        req = remoteDrive.files.get({ fileId: '123', madeThisUp: 'hello' }, utils_1.default.noop);
        assert.equal(req.uri.query, 'madeThisUp=hello');
    });
    it('should be set if params passed are aliased names', () => {
        let req = localDrive.files.get({ fileId: '123', resource_: 'hello' }, utils_1.default.noop);
        assert.equal(req.uri.query, 'resource=hello');
        req = remoteDrive.files.get({ fileId: '123', resource_: 'hello' }, utils_1.default.noop);
        assert.equal(req.uri.query, 'resource=hello');
    });
